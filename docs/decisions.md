...
| 2026-09-15 | Connect | The signed-out landing page's primary action is "Connect", not a "Sign in" link. With script, pressing it swaps the sign-in form in where the button stood and focuses the handle field, so signing in starts on the page the reader is on; the field arrives with a wash of the accent that settles over a second, the site's one animation, colour only, cut short under reduced motion. Without script it is the link to the sign-in page it always was. |
