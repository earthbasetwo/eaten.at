...
| 2026-09-25 | Composer: the digest and the teaser take the standard field rule — a hairline (the teaser had none) that goes to 2px of ink under the pointer and 2px of vermilion with focus (C7). | Ken, with Fable. Considered: framing the digest in vermilion top and bottom, and a vermilion rule in the margin beside the text. The standard rule was chosen for consistency, the frame kept as a fallback if it proves too quiet. |
